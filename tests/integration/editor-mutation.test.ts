...
describe('Godot editor mutation surface', () => {
    test('executes complete 12-step mutation lifecycle and error suite against live Godot 4.x editor', async () => {
        ...
        if (!godotBin)
            throw new Error('GODOT_BIN must be set by scripts/run-integration.mjs');
        ...
        const client = new Client({ name: 'mutation-integration-test', version: '0.1.0' }, { capabilities: { elicitation: { form: {} } } });
        ...
        try {
            ...
            const props = (propList.structuredContent as {
                properties: Array<{
                    name: string;
                }>;
            }).properties;
...
`;
            ...
            expect((validateBadRes.structuredContent as {
                errors: unknown[];
            ...
            const actions = (listInputRes.structuredContent as {
                actions: Array<{
                    name: string;
                }>;
            }).actions;
