...
export async function installAddon(root:string,template:string):Promise<string|undefined>{
 ...
 const readVersion=async(file:string)=>{
  try{return (await fs.readFile(file,'utf8')).match(/version\s*=\s*"([^"]+)"/)?.[1]??null;}catch{return null;}
 };
 const journal=await AddonJournal.prepare(root,entries.map(entry=>({...entry,relative:entry.relative.split(path.sep).join('/')})),await readVersion(path.join(target,'plugin.cfg')),await readVersion(path.join(template,'plugin.cfg')));
 try{await journal.publish();}catch(error){
  try{await journal.rollback();}catch{throw Object.assign(new Error('Addon update requires recovery; backups and journal retained'),{code:'ADDON_RECOVERY_REQUIRED'});}
  throw error;
 }
