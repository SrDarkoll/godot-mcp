...
export interface ParsedCommand {command:Command;projectRoot:string;godotBin:string|null;bridgePort?:number;json:boolean;limit:number;before?:string;sessionId?:string;repair?:boolean;}
...
export function usage():string{return [
 ...
 '  config [--godot <exe>] [--bridge-port <0..65535>] [--repair] [--json]',
...
export function parseCliArgs(argv:string[]):ParsedCommand{
 ...
 const parsed={...base,command:name as Command};
