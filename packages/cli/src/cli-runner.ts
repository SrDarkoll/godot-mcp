...
export async function runCli(argv=process.argv.slice(2)):Promise<number>{
 ...
 try{
  ...
  const explicitGodot=args.godotBin?path.resolve(args.godotBin):null;
  switch(args.command){
   case 'init':case 'addon.install':case 'addon.update':{
    const config=await readProjectConfig(root);
    const godot=explicitGodot??config.godotBin??process.env.GODOT_BIN??null;
   ...
   case 'doctor':{
    const report=await doctor({projectRoot:root,godotBin:explicitGodot});
   ...
   case 'config':{
    ...
    const saved=args.repair===true||Object.keys(changes).length>0;
    const repaired=args.repair?await repairProjectConfig(root,changes):null;
    const value=repaired?.config??(saved?await writeProjectConfig(root,changes):await readProjectConfig(root));
    emit({protocol:value.protocol,bridgePort:value.bridgePort,godotBin:value.godotBin,saved,restartRequired:saved,...(repaired?{backupPath:repaired.backupPath}: {})});return 0;
   }
   case 'permissions':{
