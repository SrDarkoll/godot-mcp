...
it('rejects missing, duplicate, ambiguous and command-inapplicable options',()=>{
 for(const args of [['init','--godot','--json'],['init','--godot','a','--godot','b'],['start','--json'],['status','--godot','x'],['sessions','inspect','../../x'],['sessions','list','--limit','201'],['doctor','--unknown'],['status','--tool-profile','3d'],['status','--client','cursor'],['init','--client','unknown'],['start','--tool-profile','physics'],['init','a','b']])expect(()=>parseCliArgs(args)).toThrow();
});
