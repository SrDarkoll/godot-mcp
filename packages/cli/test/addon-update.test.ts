...
it('restores already replaced files when a later addon file cannot be published',async()=>{
 ...
 const target=path.join(root,'addons/godot_mcp');await fs.mkdir(target,{recursive:true});
 await fs.writeFile(path.join(target,'a.gd'),'old a');await fs.writeFile(path.join(target,'b.gd'),'old b');
 await fs.writeFile(path.join(template,'a.gd'),'new a');await fs.writeFile(path.join(template,'b.gd'),'new b');
 const rename=fs.rename;let failed=false;
 const spy=vi.spyOn(fs,'rename').mockImplementation(async(...args)=>{
  if(String(args[1])===path.join(target,'b.gd')&&!failed){failed=true;throw new Error('Injected disk failure');}
  return rename(...args);
 });
 try{await expect(installAddon(root,template)).rejects.toThrow();}finally{spy.mockRestore();}
...
it('resumes compensation from a retained journal including files created by the interrupted update',async()=>{
 ...
 const journal=await AddonJournal.prepare(root,[
 ...
 await fs.writeFile(path.join(target,'a.gd'),'new a');await fs.writeFile(path.join(target,'new.gd'),'new file');
...
it('retains journal and all backups when an external edit conflicts with addon compensation',async()=>{
 ...
 const journal=await AddonJournal.prepare(root,[{relative:'a.gd',before:Buffer.from('old'),after:Buffer.from('new')}],null,null);
