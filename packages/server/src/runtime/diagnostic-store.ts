...
interface RunLog {last:number;dropped:number;sourceDropped:number;bytes:number;degraded:boolean;errorCount:number;warningCount:number;outputCount:number;}
export class DiagnosticStore {
  ...
  register(runId:string):void {if(!this.runs.has(runId))this.runs.set(runId,{last:0,dropped:0,sourceDropped:0,bytes:0,degraded:false,errorCount:0,warningCount:0,outputCount:0});}
  append(input:DiagnosticBatch):Promise<void> {
    ...
    const task=this.queue.then(async()=>{
      ...
      for(const entry of batch.entries){if(entry.sequence<=run.last)continue;run.last=entry.sequence;accepted.push(entry);this.entries.push(entry);if(entry.kind==='error')run.errorCount++;else if(entry.kind==='warning')run.warningCount++;else run.outputCount++;}
  ...
  counts(runId:string):{errorCount:number;warningCount:number;outputCount:number}{
    const run=this.runs.get(runId);if(!run)throw new BridgeRpcError('RUN_NOT_FOUND','Run is not in this session');
    return {errorCount:run.errorCount,warningCount:run.warningCount,outputCount:run.outputCount};
  }
