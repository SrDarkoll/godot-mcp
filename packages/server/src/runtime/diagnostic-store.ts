  ...
  append(input:DiagnosticBatch):Promise<void> {
    ...
    const task=this.queue.then(async()=>{
      ...
      try {
        ...
        try {const stat=await fs.lstat(file);if(!stat.isFile()||stat.isSymbolicLink()||stat.nlink!==1)throw new Error('Log is a link');}catch(e){if((e as NodeJS.ErrnoException).code!=='ENOENT')throw e;}
