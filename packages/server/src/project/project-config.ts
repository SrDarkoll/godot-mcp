...
export async function repairProjectConfig(root:string,changes:{godotBin?:string|null;bridgePort?:number;toolProfile?:ToolProfile}):Promise<{config:ProjectConfig;backupPath:string|null}>{
 ...
 const config=ConfigSchema.parse({...object,
  ...
  toolProfile:ConfigSchema.shape.toolProfile.safeParse(object.toolProfile).success?object.toolProfile:defaults.toolProfile,
  ...changes,
 });
