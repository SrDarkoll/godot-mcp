...
function edits(before: string[], after: string[]): Line[] {
  ...
  const next = after.slice(prefix, after.length - suffix);
  ...
  if ((old.length + 1) * (next.length + 1) <= 4000000) {
    ...
    const table = new Uint32Array((old.length + 1) * stride);
    ...
    while (i < old.length || j < next.length) {
      if (i < old.length && j < next.length && old[i] === next[j]) {
        ...
        operations.push({ kind: '+', text: next[j++]! });
      }
    }
  } else {
    operations.push(
...
export async function diffTransaction(
  ...
  let remainingBytes = options.max_diff_bytes;
  const files = [];
  ...
  for (const after of record.after) {
    ...
    const b = await snapshots.bytes(record, 'after', after);
    const old = text(a);
    const next = text(b);
    const entry = {
      path: after.path,
    ...
    for (let i = 0; i < ops.length; i++) {
      if (ops[i]!.kind !== ' ') {
        ...
        const end = Math.min(ops.length, i + options.context_lines + 1);
    ...
    for (const [start, end] of ranges) {
      ...
      const oldCount = chunk.filter((l) => l.kind !== '+').length;
      const newCount = chunk.filter((l) => l.kind !== '-').length;
      if (
        !append(
      ...
      for (const line of chunk) {
        if (!append(line.kind + redact(line.text))) break;
      }
