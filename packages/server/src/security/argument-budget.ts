...
export function validateArgumentBudget(input: unknown): void {
  ...
  const fail = () => {
    throw new BridgeRpcError('ARGUMENT_TOO_LARGE', 'Arguments exceed JSON depth, item, byte or cycle limits');
  };
  ...
  while (stack.length) {
    ...
    if (typeof value === 'string') {
      ...
      if (Array.isArray(value)) {
        ...
        if (keys.length > 50000 - items) {
          fail();
        }
        for (const key of keys) {
          bytes += Buffer.byteLength(key);
          stack.push({ value: (value as Record<string, unknown>)[key], depth: entry.depth + 1 });
        }
      }
    }
