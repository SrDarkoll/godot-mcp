...
function validatePng(payload: CapturePayload): Buffer {
  ...
  if (bytes.length > MAX_CAPTURE_BYTES)
    throw new BridgeRpcError('CAPTURE_TOO_LARGE', 'PNG exceeds 16 MiB');
  const invalid = () => {
    throw new BridgeRpcError('INVALID_CAPTURE_PAYLOAD', 'Invalid PNG capture');
  };
  if (
    bytes.toString('base64') !== payload.png_base64 ||
    bytes.length < 45 ||
  ...
  while (offset + 12 <= bytes.length) {
    ...
    if (
      ...
      (type !== 'IHDR' ||
        length !== 13 ||
        bytes.readUInt32BE(16) !== payload.width ||
        bytes.readUInt32BE(20) !== payload.height)
    )
      invalid();
...
export class ScreenshotStore {
  ...
  private async persist(input: SaveCaptureInput): Promise<CaptureResult> {
    ...
    if (
      !parsed.success ||
      !metadata.success ||
      (input.type !== 'editor_3d'
        ? parsed.data.viewport_index !== null
        : parsed.data.viewport_index === null) ||
      (input.type === 'game' &&
        !GameCapturePayloadSchema.safeParse({ ...input.payload, run_id: input.runId }).success)
    ) {
      throw new BridgeRpcError('INVALID_CAPTURE_PAYLOAD', 'Invalid capture metadata');
    }
    const payload = parsed.data;
    const bytes = validatePng(payload);
    if ((await this.sessions.read(this.session.id)).endedAt)
      throw new BridgeRpcError('SESSION_CLOSED', 'Session has ended');
    const slug =
      metadata.data.label
        .normalize('NFKD')
        .replace(/[\u0300-\u036f]/g, '')
    ...
    for (;;) {
      ...
      try {
        ...
        throw new BridgeRpcError(
          ...
          'Unable to persist PNG; any partial artifact is retained',
        );
      }
    }
    const screenshot: ScreenshotRecord = {
      ...
      type: input.type,
      path: relative,
      runId: input.type === 'game' ? input.runId! : null,
      scene: payload.scene,
      ...
      viewportIndex: payload.viewport_index,
    };
    const checkpoint = metadata.data.checkpoint
      ? {
          id: randomUUID(),
    ...
    await this.sessions.update(this.session.id, (m) => ({
      ...m,
      screenshots: [...m.screenshots, screenshot],
