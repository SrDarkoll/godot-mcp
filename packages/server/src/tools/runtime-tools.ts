...
import {
  ScenePathSchema,
  RuntimeTreeParamsSchema,
  RuntimeInspectParamsSchema,
  RuntimePropertyParamsSchema,
...
export function registerRuntimeTools(
  ...
  const register = (
    ...
    server.registerTool(
      name,
      {
        description: `Structured Godot ${name} operation on the current runtime session.`,
        inputSchema: schema,
      },
      async (args) => {
        try {
  ...
  register('project.run_scene', z.strictObject({ path: ScenePathSchema.optional() }), (args) =>
    runtime.run(args.path ? { target: 'path', path: args.path } : { target: 'current' }),
  );
  ...
  register('runtime.scene_tree', RuntimeTreeParamsSchema, (args) =>
    runtime.request('runtime.scene_tree', args),
  );
  register('runtime.inspect_node', RuntimeInspectParamsSchema, (args) =>
    runtime.request('runtime.inspect_node', args),
  );
  register('runtime.get_property', RuntimePropertyParamsSchema, (args) =>
    runtime.request('runtime.get_property', args),
  );
