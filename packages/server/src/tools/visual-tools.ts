...
interface VisualBridge { connected:boolean; capabilities:AddonCapabilities|null; rpc:Pick<RpcRouter,'call'>; }
interface RuntimeCapture {request(method:string,params?:Record<string,unknown>,timeout?:number):Promise<any>;}
export class VisualTools {
  private readonly screenshots:ScreenshotStore;
  private queue:Promise<unknown> = Promise.resolve();
  private closed = false;
  private pending = 0;
  constructor(private readonly session:Session,private readonly sessions:SessionStore,private readonly bridge:VisualBridge,private readonly runtime?:RuntimeCapture) {
    this.screenshots = new ScreenshotStore(session,sessions);
  }
  capture(type:'editor_2d'|'editor_3d'|'game',input:unknown):Promise<{result:CaptureResult;data:string}> {
    if (this.closed) return Promise.reject(new BridgeRpcError('SESSION_CLOSED','Session is closing'));
    if (this.pending >= 64) return Promise.reject(new BridgeRpcError('BUSY','Capture queue is full'));
    this.pending++;
    const result = this.queue.then(async () => {
      ...
      try {
        ...
        const rpcParams = type === 'editor_2d' ? {} : {viewport_index:Capture3DParamsSchema.parse(input).viewport_index};
        const payload = CapturePayloadSchema.safeParse(await this.bridge.rpc.call(tool,rpcParams));
        if (!payload.success) throw new BridgeRpcError('INVALID_CAPTURE_PAYLOAD','Editor returned invalid capture metadata');
        ...
        await this.sessions.update(this.session.id,m=>({...m,errors:[...m.errors,
          {timestamp:new Date().toISOString(),tool,code,message:'Visual capture did not complete'}]})).catch(()=>{});
        throw error instanceof BridgeRpcError ? error : new BridgeRpcError(code,'Visual capture did not complete');
      }
    });
