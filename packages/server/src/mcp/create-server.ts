...
import { registerDebuggerTools } from './register-debugger-tools.js';
...
export function createMcpServer(ctx: McpServerContext): McpServer {
    ...
    const debuggerService = ctx.debugger ?? new DebuggerService(ctx.session, runtime, ctx.bridge, () => new TcpDapTransport());
