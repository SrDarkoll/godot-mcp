...
const nonnegativeVector=z.object({x:z.number().int().nonnegative(),y:z.number().int().nonnegative()});
export function registerTilesetTools(registrar:ToolRegistrar,rpc:BridgeServer['rpc']):void{
  ...
  registrar.registerTool('tileset.add_atlas_source',{description:'Add a project texture as a TileSetAtlasSource with Undo/Redo support.',inputSchema:z.object({node_path:z.string().min(1),texture_path:z.string().min(1).max(2048),texture_region_size:TilePositiveSizeSchema,source_id:z.number().int().nonnegative().optional(),margins:nonnegativeVector.optional(),separation:nonnegativeVector.optional()})},async args=>{try{return toolSuccess(await addTilesetAtlasSource(rpc,omitUndefinedValues(args)));}catch(error){return toolError(error);}});
  registrar.registerTool('tileset.inspect_atlas_source',{description:'Inspect one TileSetAtlasSource and its base atlas tiles.',inputSchema:z.object({node_path:z.string().min(1),source_id:z.number().int().nonnegative()})},async args=>{try{return toolSuccess(await inspectTilesetAtlasSource(rpc,args));}catch(error){return toolError(error);}});
  registrar.registerTool('tileset.create_atlas_tiles',{description:'Create up to 4096 base atlas tiles atomically with Undo/Redo.',inputSchema:z.object({node_path:z.string().min(1),source_id:z.number().int().nonnegative(),tiles:z.array(TileAtlasTileCreateSchema).min(1).max(4096)})},async args=>{try{return toolSuccess(await createTilesetAtlasTiles(rpc,args));}catch(error){return toolError(error);}});
  registrar.registerTool('tileset.remove_source',{description:'Remove an unused TileSet source conservatively with Undo/Redo support.',inputSchema:z.object({node_path:z.string().min(1),source_id:z.number().int().nonnegative()})},async args=>{try{return toolSuccess(await removeTilesetSource(rpc,args));}catch(error){return toolError(error);}});
}
