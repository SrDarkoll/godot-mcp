...
func _project_scene_changed(scene: Node) -> void:
    if is_instance_valid(_bridge): _bridge.queue_project_event("scene.changed",scene.scene_file_path if scene else null)
func _project_scene_saved(path_value: String) -> void:
    if is_instance_valid(_bridge): _bridge.queue_project_event("scene.saved",path_value)
func _project_filesystem_changed() -> void:
    if is_instance_valid(_bridge): _bridge.queue_project_event("filesystem.changed")
func _project_reimported(_paths: PackedStringArray) -> void:
    if is_instance_valid(_bridge): _bridge.queue_project_event("import.finished")
